...
class dog:
    ...
    class Player(Human):
        ...
        def actions(self):
            ...
            print("4. купить собаку")
            print("5. Баланс")

        def day(self, petShop=None):
            choice = int(input("-> "))
            if choice == 1:
                self.rest()
            elif choice == 2:
                ...
                print("Собаки: ")
                for i in range(len(petShop)):
                    print(f"{i + 1}. {petShop[i].name}")
                choice = int(input("-> "))
                if choice > 0 and choice < len(petShop):
                    petShop[choice - 1].buy(self)
    ...
    def game_start():
        global house, autopark, dog, thief
        house = House()
    ...
    while True:
        ...
        for human in house.humans:
            ...
            if random.randint(1, 100) <= 10:
                print(f"{human.name} хочет купить собаку")
                random.choice(petShop).buy(human)
            random.choice(actions)()
